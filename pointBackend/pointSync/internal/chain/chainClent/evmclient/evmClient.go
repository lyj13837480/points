...
func (s *Service) BlockWithHash(ctx context.Context, blockNumber uint64) (common.Hash, error) {
	blockWithTxs, err := s.client.HeaderByNumber(ctx, big.NewInt(int64(blockNumber)))
	if err != nil {
		return common.Hash{}, errors.Wrap(err, "failed on get evm block")
	}
	return blockWithTxs.Hash(), nil
}
