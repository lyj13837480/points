...
type UserBalanceChange struct {
	...
	Amount       *big.Int  `gorm:"column:amount;type:decimal(65,0);not null;comment:变动金额正数表示增加，负数表示减少" json:"amount"`
	BalanceAfter string    `gorm:"column:balance_after;type:decimal(65,0);not null;comment:变动后的余额" json:"balance_after"`
